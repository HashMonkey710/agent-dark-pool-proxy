...
// ../agent-kit-identity/src/utils/signatures.ts
...
// ../agent-kit-identity/src/utils/address.ts
...
// ../agent-kit-identity/src/utils/domain.ts
function normalizeDomain(domain) {
  return domain?.trim?.().toLowerCase?.() ?? "";
}

// ../agent-kit-identity/src/config/erc8004.ts
...
// ../agent-kit-identity/src/abi/IdentityRegistry.json
...
// ../agent-kit-identity/src/abi/ReputationRegistry.json
...
// ../agent-kit-identity/src/abi/ValidationRegistry.json
...
// ../agent-kit-identity/src/abi/types.ts
...
// ../agent-kit-identity/src/registries/identity.ts
...
// ../agent-kit-identity/src/registries/reputation.ts
...
// ../agent-kit-identity/src/registries/validation.ts
...
// ../agent-kit-identity/src/init.ts
async function createAgentIdentity(options = {}) {
  ...
  const resolvedChainId = chainId ?? (typeof env === "object" && env?.CHAIN_ID ? parseInt(env.CHAIN_ID) : void 0) ?? DEFAULT_CHAIN_ID;
  const resolvedRegistryAddress = registryAddress ?? (typeof env === "object" && env?.IDENTITY_REGISTRY_ADDRESS ? env.IDENTITY_REGISTRY_ADDRESS : void 0) ?? getRegistryAddresses(resolvedChainId).IDENTITY_REGISTRY;
  ...
  if (viemFactory) {
    try {
      const resolvedChainId2 = chainId ?? (env && typeof env === "object" ? parseInt(env.CHAIN_ID || "84532") : 84532);
      const resolvedRpcUrl = rpcUrl ?? (env && typeof env === "object" ? env.RPC_URL : void 0);
      if (resolvedRpcUrl) {
        const vClients = await viemFactory({
          chainId: resolvedChainId2,
          rpcUrl: resolvedRpcUrl,
          env: env ?? {}
        });
        if (vClients?.publicClient) {
          const registryAddresses = getRegistryAddresses(resolvedChainId2);
...
export { AP2_EXTENSION_URI, CART_MANDATE_DATA_KEY, CONTACT_ADDRESS_DATA_KEY, DEFAULT_CHAIN_ID, DEFAULT_NAMESPACE, DEFAULT_TRUST_MODELS, INTENT_MANDATE_DATA_KEY, PAYMENT_MANDATE_DATA_KEY, PAYMENT_METHOD_DATA_KEY, SUPPORTED_CHAINS, ZERO_ADDRESS, accountFromPrivateKey, bootstrapIdentity, bootstrapTrust, buildDomainProofMessage, buildFeedbackAuthMessage, buildManifest, buildMetadataURI, buildTrustConfigFromIdentity, buildValidationRequestMessage, configureAgentKit, createAgentApp, createAgentIdentity, createAxLLMClient, createIdentityRegistryClient, createReputationRegistryClient, createRuntimePaymentContext, createValidationRegistryClient, createX402Fetch, createX402LLM, generateAgentMetadata, getAgentKitConfig, getRegistryAddress, getRegistryAddresses, getTrustConfig, hashMessageEIP191, isChainSupported, isERC8004Registry, makeViemClientsFromEnv, normalizeAddress, normalizeDomain, paymentsFromEnv, recoverSigner, registerAgent, resetAgentKitConfigForTesting, resolveEntrypointPrice, sanitizeAddress, signAgentDomainProof, signDomainProof, signFeedbackAuth, signMessageWithViem, signTypedDataWithViem, signValidationRequest, toCaip10, toJsonSchemaOrUndefined, verifySignature, withPayments, x402LLM };
